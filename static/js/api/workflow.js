...
const ApiFetchFlows = async () => {
    const api_url = V.build_api_url('flows', 'workflows')
    const res = await fetch(`${api_url}/${getSelectedProjectId()}`, {
        method: 'GET',
    })
    return res.json();
...
const ApiFetchFlowById = async (flowId) => {
    const api_url = V.build_api_url('flows', 'workflow')
    const res = await fetch(`${api_url}/${getSelectedProjectId()}/${flowId}`, {
        method: 'GET',
    })
    return res.json();
}